...
mkdir -p "$stage/images" "$stage/chart" "$stage/models/hushmark-tr" "$stage/manifests"
...
for model_file in gliner_config.json gliner_config.source.json model.onnx tokenizer.json tokenizer_config.json; do
  source_file="$repo_dir/models/hushmark-tr/$model_file"
  [[ -f "$source_file" ]] || { echo "verified model file is missing: $source_file" >&2; exit 1; }
  cp "$source_file" "$stage/models/hushmark-tr/$model_file"
done
