#!/usr/bin/env python3
"""Apply the hardware and quality gate before a full BERTurk training run."""
...
def pilot_verdict(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reasons: list[str] = []
    if manifest.get("architecture") != "berturk-fixed-span-ner":
        reasons.append("pilot used the wrong architecture")
    ...
    if manifest.get("max_steps") != 1000 or manifest.get("optimizer_steps") != 1000:
        reasons.append("pilot did not complete exactly 1000 optimizer steps")
    if manifest.get("stop_reason") != "max-steps":
        reasons.append("pilot did not stop at its max-step boundary")
    for field in ("mean_loss", "final_loss", "development_macro_f1"):
    ...
    if manifest.get("pilot_quality_pass") is not True:
        reasons.append("pilot did not meet the development macro-F1 floor")
