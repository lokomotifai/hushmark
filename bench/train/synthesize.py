...
def main() -> int:
    ...
    if args.check:
        first = synthesize(seed=args.seed, count=args.examples)
        second = synthesize(seed=args.seed, count=args.examples)
        if first != second:
    ...
    summary = synthesize(seed=args.seed, count=args.examples, output_path=args.output)
    metadata_path = args.output.with_suffix(".metadata.json")
    metadata_path.write_text(
        json.dumps({"seed": args.seed, **asdict(summary)}, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
